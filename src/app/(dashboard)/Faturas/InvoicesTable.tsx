...
export default function InvoicesTable() {
  ...
  }, [invoices.length, totalPages, page]);
  ...
  const handleCreate = async (data: Omit<Invoice, "id">) => {
    const created = await invoicesService.create(data);
    setInvoices((prev) => [...prev, created]);
    setFormOpen(false);
    setPage(Math.ceil((invoices.length + 1) / pageSize));
  };
  ...
  const handleUpdate = async (id: string, data: Partial<Invoice>) => {
    ...
    setInvoices(
      (prev) => prev.map((inv) => (inv.id === id ? updated : inv)) as Invoice[],
    );

    setEditingInvoice(null);
  };

  // Buscar detalhes (lazy)
  useEffect(() => {
  ...
  return (
    <Container>
      <div className="w-full">
        ...
        {/* Paginação (mantida no teu padrão visual) */}
      ...
      >
        {!selectedInvoice ? (
          ...
          <div className="space-y-3 text-sm text-gray-700">
            <div>
              <strong>Cliente:</strong> {selectedInvoice.clientId}
            </div>
            <div>
              <strong>Valor:</strong> €{selectedInvoice.amount.toLocaleString()}
            </div>
            <div>
